...
func TestMerge(t *testing.T) {
	...
	_, _, err = Merge(segsToMerge, []*roaring.Bitmap{nil, nil}, "/tmp/scorch3.zap", 1024, NewSnappy())
	if err != nil {
		t.Fatal(err)
	}
...
func testMergeWithEmptySegments(t *testing.T, before bool, numEmptySegments int) {
	...
	for i := 0; i < numEmptySegments; i++ {
		...
		emptySegment, _, _ := AnalysisResultsToSegmentBase([]*index.AnalysisResult{}, 1024, NewSnappy())
	...
	_, _, err = Merge(segsToMerge, drops, "/tmp/scorch3.zap", 1024, NewSnappy())
	if err != nil {
		t.Fatal(err)
	}
...
func testMergeWithSelf(t *testing.T, segCur *Segment, expectedCount uint64) {
	...
	for i := 0; i < 10; i++ {
		...
		_, _, err := Merge(segsToMerge, []*roaring.Bitmap{nil, nil}, "/tmp/"+fname, 1024, NewSnappy())
		if err != nil {
			t.Fatal(err)
		}
...
func testMergeAndDropSegments(t *testing.T, segsToMerge []*Segment, docsToDrop []*roaring.Bitmap, expectedNumDocs uint64) {
	...
	_, _, err := Merge(segsToMerge, docsToDrop, "/tmp/scorch-merged.zap", 1024, NewSnappy())
	if err != nil {
		t.Fatal(err)
	}
...
func TestMergeBytesWritten(t *testing.T) {
	...
	_, nBytes, err := Merge(segsToMerge, []*roaring.Bitmap{nil, nil}, "/tmp/scorch3.zap", 1024, NewSnappy())
	if err != nil {
		t.Fatal(err)
	}
